    ...
    *dns = (NMDnsServer) {0};
    ...
    if (dns->scheme != NM_DNS_URI_SCHEME_TLS && dns->servername) {
        g_set_error_literal(error,
                            NM_CONNECTION_ERROR,
                            NM_CONNECTION_ERROR_INVALID_PROPERTY,
                            _("server name is only supported for DNS-over-TLS"));
        return FALSE;
    }
    ...
    if (dns->interface[0]
        ...
        g_set_error_literal(error,
                            NM_CONNECTION_ERROR,
                            NM_CONNECTION_ERROR_INVALID_PROPERTY,
                            _("interface is only supported for IPv6 link-local addresses"));
        return FALSE;
    }
