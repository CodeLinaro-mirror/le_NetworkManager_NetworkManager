...
static void
activate_slave_or_children_connections(NMPolicy *self,
                                       NMDevice *device,
                     ...
                     gpointer            user_data)
{
    ...
    case NM_DEVICE_STATE_FAILED:
        ...
        if (sett_conn && old_state >= NM_DEVICE_STATE_PREPARE
            ...
            case NM_DEVICE_STATE_REASON_DEPENDENCY_FAILED:
                 ...
                 * able to reconnect until the connection it depends on activates
                 * again; when this happens, the master/parent clears the blocked
                 * reason for all its dependent devices in activate_slave_or_children_connections()
    ...
    case NM_DEVICE_STATE_ACTIVATED:
        if (nm_device_get_device_type(device) == NM_DEVICE_TYPE_OVS_INTERFACE) {
            ...
            * delay unblocking the children and schedule them for activation until parent is activated */
            activate_slave_or_children_connections(self, device, TRUE);
        }
        if (sett_conn) {
    ...
    case NM_DEVICE_STATE_PREPARE:
        /* Reset auto-connect retries of all slaves or children and schedule them for
         * activation. */
        activate_slave_or_children_connections(self, device, FALSE);
