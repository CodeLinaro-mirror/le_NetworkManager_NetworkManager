...
It's mostly automated by [contrib/fedora/rpm/release.sh]().

Before running the script:
- For stable releases, remember to backport all commits with "Fixes:" tag that
  are applicable. Use `contrib/scripts/find-backports` to find them.
...
- For stable releases:
  - The official documentation must be updated on the website when there is a new
    stable release. Use the following script from the website's repo:
    https://gitlab.freedesktop.org/NetworkManager/networkmanager.pages.freedesktop.org/-/blob/main/scripts/import-docs.sh.
  - The release should be announced on the mailing list.
...
- There are no stable branches or development versions. Odd and even numbers
...
6. Announce the release on the mailing list and other channels.
